...
const AppName string = "ics-cloud-controller-manager"
...
func main() {
	...
	klog.V(1).Infof("ics-cloud-controller-manager version: %s", version)

	// Set cloud-provider flag to vsphere
	command.Flags().VisitAll(func(flag *pflag.Flag) {
		if flag.Name == "cloud-provider" {
			flag.Value.Set(ics.ProviderName)
			flag.DefValue = ics.ProviderName
			return
		}
	})
