...
CCM_BIN_NAME := ics-cloud-controller-manager
...
##                                   DIST                                     ##
################################################################################
DIST_CCM_NAME := cloud-provider-ics-$(VERSION)
...
clean:
	...
	@rm -f $(CCM_BIN) $(CCM_BIN).sha256 \
	  cloud-provider-ics-*.tar.gz cloud-provider-ics-*.zip \
		image-*.tar image-*.d
...
unit unit-test:
	env -u ICS_SERVER -u ICS_PASSWORD -u ICS_USER go test $(TEST_FLAGS) -tags=unit $(PKGS_WITH_TESTS)
build-unit-tests:
...
ifeq (true,$(DOCKER_IN_DOCKER_ENABLED))
	...
	docker run -it --rm  \
	  ...
	  -v "$(PWD)":/go/src/k8s.io/cloud-provider-ics \
